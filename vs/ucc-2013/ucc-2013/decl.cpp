...
static AstInitializer ParseInitializer ( void )
{
	...
	CREATE_AST_NODE ( init, Initializer, AstInitializer );
	// { {1,2,3},4,5,6}
...
static AstInitDeclarator ParseInitDeclarator ( void )
{
	...
	CREATE_AST_NODE ( initDec, InitDeclarator, AstInitDeclarator );
...
static AstDeclarator ParseDirectDeclarator ( int kind )
{
	...
	if (CurrentToken == TK_LPAREN)
	{
		...
		if (t != TK_ID && t != TK_LPAREN && t != TK_MUL)
		{
			...
			EndPeekToken ();
			CREATE_AST_NODE ( dec, NameDeclarator, AstDeclarator );
		}
		else
		{
			EndPeekToken ();
...
static AstParameterDeclaration ParseParameterDeclaration ( void )
{
	...
	CREATE_AST_NODE ( paramDecl, ParameterDeclaration, AstParameterDeclaration );
...
AstParameterTypeList ParseParameterTypeList ( void )
{
	...
	CREATE_AST_NODE ( paramTyList, ParameterTypeList, AstParameterTypeList );
...
static AstDeclarator ParsePostfixDeclarator ( int kind )
{
	...
	while (1)
	{
		if (CurrentToken == TK_LBRACKET)
		{
			...
			CREATE_AST_NODE ( arrDec, ArrayDeclarator, AstArrayDeclarator );
			arrDec->dec = dec;
		...
		else if (CurrentToken == TK_LPAREN)
		{
			...
			CREATE_AST_NODE ( funcDec, FunctionDeclarator, AstFunctionDeclarator );
			funcDec->dec = dec;
...
static AstDeclarator ParseDeclarator ( int kind )
{
	if (CurrentToken == TK_MUL)
	{
		...
		CREATE_AST_NODE ( ptrDec, PointerDeclarator, AstPointerDeclarator );
		tail = &ptrDec->tyQuals;
		...
		{
			CREATE_AST_NODE ( tok, Token,AstToken );
...
static AstStructDeclarator ParseStructDeclarator ( void )
{
	...
	CREATE_AST_NODE ( stDec, StructDeclarator, AstStructDeclarator );
	// [declarator]
...
static AstStructDeclaration ParseStructDeclaration ( void )
{
	...
	CREATE_AST_NODE ( stDecl, StructDeclaration, AstStructDeclaration );
...
static AstStructSpecifier ParseStructOrUnionSpecifier ( void )
{
	...
	CREATE_AST_NODE ( stSpec, StructSpecifier, AstStructSpecifier );
...
static AstEnumerator ParseEnumerator ( void )
{
	...
	CREATE_AST_NODE ( enumer, Enumerator, AstEnumerator );
...
static AstEnumSpecifier ParseEnumSpecifier ( void )
{
	...
	CREATE_AST_NODE ( enumSpec, EnumSpecifier, AstEnumSpecifier );
...
static AstSpecifiers ParseDeclarationSpecifiers ( void )
{
	...
	CREATE_AST_NODE ( specs, Specifiers, AstSpecifiers );
	...
	switch (CurrentToken)
	{
		...
		case TK_ID:
			...
			if (!seeTy && IsTypedefName ( ( char* )TokenValue.p ))
			{
				...
				CREATE_AST_NODE ( tname, TypedefName, AstTypedefName );
...
AstTypeName ParseTypeName ( void )
{
	...
	CREATE_AST_NODE ( tyName, TypeName, AstTypeName );
...
static AstDeclaration ParseCommonHeader ( void )
{
	...
	CREATE_AST_NODE ( decl, Declaration, AstDeclaration );
	// declaration-specifiers
...
static AstNode ParseExternalDeclaration ( void )
{
	...
	*/
	...
	if (fdec != NULL)
	{
		...
		*/
		CREATE_AST_NODE ( func, Function, AstFunction );
...
AstTranslationUnit ParseTranslationUnit ( char *filename )
{
	...
	*/
	...
	CREATE_AST_NODE ( transUnit, TranslationUnit, AstTranslationUnit );
