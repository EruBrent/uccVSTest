...
static AstExpression ScalePointerOffset ( AstExpression offset, int scale )
{
	...
	CREATE_AST_NODE ( expr, Expression, AstExpression );
...
static AstExpression CheckPrimaryExpression ( AstExpression expr )
{
	...
	if (expr->op == OP_STR)
	{
		...
		*/
		...
		expr->op = OP_ID;
		expr->val.p = AddString ( expr->ty, (String)expr->val.p, &expr->coord );
		expr->lvalue = 1;
	...
	if (p == NULL)
	{
		...
		p = AddVariable ((char*) expr->val.p, T ( INT ), Level == 0 ? 0 : TK_AUTO, &expr->coord );
		expr->ty = T ( INT );
		expr->lvalue = 1;
	}
...
static AstExpression CheckFunctionCall ( AstExpression expr )
{
	...
	if (expr->kids[0]->op == OP_ID && LookupID ((char*) expr->kids[0]->val.p ) == NULL)
	{
		...
		expr->kids[0]->val.p = AddFunction ((char*) expr->kids[0]->val.p, DefaultFunctionType, TK_EXTERN, &expr->coord );
	}
	else
	{
...
static AstExpression CheckMemberAccess ( AstExpression expr )
{
	...
	fld = LookupField ( Unqual ( ty ), (char*)expr->val.p );
	if (fld == NULL)
	{
...
static AstExpression TransformIncrement ( AstExpression expr )
{
	...
	val.i[1] = 0; val.i[0] = 1;
	CREATE_AST_NODE ( casgn, Expression, AstExpression );
				...
				*/
				...
				if (!expr->kids[0]->isarray && expr->ty->categ != ARRAY)
				{
					...
					*/
					...
					CREATE_AST_NODE ( deref, Expression, AstExpression );
					CREATE_AST_NODE ( addExpr, Expression, AstExpression );
					deref->op = OP_DEREF;
...
static AstExpression CheckAssignmentExpression ( AstExpression expr )
{
	...
	*/
	if (expr->op != OP_ASSIGN)
	{
		...
		CREATE_AST_NODE ( lopr, Expression, AstExpression );
...
static AstExpression CastExpression ( Type ty, AstExpression expr )
{
	...
	CREATE_AST_NODE ( cast, Expression, AstExpression );
