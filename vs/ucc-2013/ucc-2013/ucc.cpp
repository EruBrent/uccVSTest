...
char* FileName(char *name, char *ext)
{
	...
	str = (char*)Alloc(len + strlen(ext)+1);
	...
	for (i = j = 0; cmd[i] != NULL; i++)
	{
		...
		if (s && isdigit(s[1]))
		{
			...
			if (p = lists[d - 1])
			{
				...
				*/
				Command[j] =(char*) Alloc(strlen(cmd[i]) + strlen(p->str) - 1);
...
int Driver ( int argc, char *argv[] )
{
	...
	SetupToolChain ();
	Command = ( char** )Alloc ( (argc + 60) * sizeof ( char * ) );
	Command[0] = NULL;
