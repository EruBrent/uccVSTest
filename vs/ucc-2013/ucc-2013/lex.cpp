...
static void ScanPPLine ( void )
{
	...
	else if (strncmp ( (char*)CURSOR, "line", 4 ) == 0)
	{
		CURSOR += 4;
		while (*CURSOR == ' ' || *CURSOR == '\t')
		{
...
int SkipWin32Declares ( void )
{
	...
	*/
	if (strncmp ( (char*)CURSOR, DECLSPEC_PREFIX, strlen ( DECLSPEC_PREFIX ) ) == 0)
	{
		int lpCount = 1;
	...
	*/
	if (strncmp ( (char*)CURSOR, WIN32_ASM_PREFIX, strlen ( WIN32_ASM_PREFIX ) ) == 0)
	{
		int rbraceCount = 0;
...
again:
	...
	if (ExtraWhiteSpace != NULL)
	{
		...
		FOR_EACH_ITEM ( char*, p, ExtraWhiteSpace )
			if (strncmp ( (char*)CURSOR, p, strlen ( p ) ) == 0)
			{
				CURSOR += strlen ( p );
				goto again;
			}
