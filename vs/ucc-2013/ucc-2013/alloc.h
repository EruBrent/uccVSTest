...
#define DO_ALLOC(p,type)    ((p) = (##type) HeapAllocate(CurrentHeap, sizeof *(p)))
#define ALLOC(p,type)   memset(DO_ALLOC(p,type), 0, sizeof *(p))
#define CALLOC(p,type)   memset(DO_ALLOC(p,type), 0, sizeof *(p))
